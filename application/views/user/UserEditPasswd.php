...
if(isset($userDtl['userID'])){ ?>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input  type="text" class="form-control" name="username" id="username" value="<?php echo $userDtl['username'] ?>" readonly>
							</div>
					...
					<div class="col-md-12">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input  type="email" class="form-control" name="useremail" id="useremail" value="<?php echo $userDtl['useremail'] ?>" readonly>
							</div>
					...
					<div class="col-md-12">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input  type="password" class="form-control" name="old_passwwd" id="old_passwwd" autocomplete="on" value="" >
							</div>
							<div class="col-sm-2 text-danger" id="error_ole_passwd"></div>
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input  type="password" class="form-control" name="new_passwd" id="new_passwd" value="" disabled>
							</div>
							<div class="col-sm-2" id="error_new_passwd"></div>
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input  type="password" class="form-control" name="conf_new_passwd" id="conf_new_passwd" value="" disabled="" >
							</div>
							<div class="col-sm-2" id="error_conf_new_passwd"></div>
						</div>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		function checkOldPwd() {
			$('#old_passwwd').keyup(function(){
				$.ajax({
					url: '<?php echo base_url();?>index.php/Profile/checkOldPwd/',
					type: 'POST',
					dataType: 'json',
					data: { username: $('#username').val(),old_passwwd: $('#old_passwwd').val()},
					success: function(rs)
					{
						if(rs.length > 0){
							$('#new_passwd').removeAttr('disabled');
							$('#conf_new_passwd').removeAttr('disabled');
							$('#error_ole_passwd').html('<i class="fa fa-check text-success fa-2x" aria-hidden="true"></i>');
						}else{
							$('#error_ole_passwd').html('รหัสผ่านเดิม ไม่ถูกต้อง');
						}
					},
					error: function(){
						alert("Not Found !");
		...
		function checkNewPwd() {
			$('#conf_new_passwd').keyup(function(){
				if($('#new_passwd').val() == $(this).val() && $('#new_passwd').val() != ''){
					$('#save').removeAttr('disabled');
					$('#error_conf_new_passwd').html('<i class="fa fa-check text-success fa-2x" aria-hidden="true"></i>');
				}else{
					$('#error_conf_new_passwd').html('ยืนยันรหัสผ่านใหม่ ไม่ตรงกัน');
				}
			});
		}
		...
		function confirmvalid()
		{
			if($('#username').val()==''){
				...
				$('#username').focus();
				return false;
			}else if($('#old_passwwd').val()==''){
				alert("Please input old_passwwd ");
				$('#old_passwwd').focus();
				return false;
